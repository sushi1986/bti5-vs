        ...
        if (future[nextSlot] == null) {
            ...
            if (current[currentSlot] == null) {
                System.out.printf("[WORKER][main][%2d] Slot not available (t%s -> s%d), conflic with (t%s - s%d)\n",
                        currentSlot, future[nextSlot].getTeam(), future[nextSlot].getSlot(), msg.getSender(),
                        msg.getNextSlot());
                return false;
            }
            else {
                if (current[currentSlot].getTeam().equals(msg.getSender())) {
                    System.out.printf("[WORKER][main][%2d][Overwriting team: %s] %s\n", currentSlot, future[nextSlot].getTeam(), msg.toString());
                    TimeSlot tmp = new TimeSlot();
                    tmp.setTeam(msg.getSender());
                    future[nextSlot] = tmp;
                    return true;
                    ...
                    System.out.printf(
                            "[WORKER][main][%2d] Slot not available (t%s -> s%d), conflic with (t%s - s%d)\n",
                            currentSlot, future[nextSlot].getTeam(), future[nextSlot].getSlot(), msg.getSender(),
                            msg.getNextSlot());
                    return false;
                }
                    ...
                    if (now.getTeam().equals(self)) { // i have to send
                        if (!sentMessage && TimeHandler.generateTimeStamp() >= (beginOfNextSlot - 30)) {
                            ...
                            if (nextSlot == -1) {
                                ...
                                Message tmp = new Message(new byte[] { 0 }, self, nextSlot,
